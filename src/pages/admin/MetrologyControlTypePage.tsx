...
export default function MetrologyControlTypePage() {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      {isMobile ? (
        // 📱 МОБИЛЬНАЯ ВЕРСИЯ: Компактный однострочный список видов контроля
        <Stack spacing={1}>
          {metrologyControlTypes.map((m) => (
            <Paper
              ...
              variant="outlined"
              sx={{
                p: 1.5, // Сжали отступы для экономии высоты экрана
                borderRadius: 2,
                bgcolor: 'background.paper',
                display: 'flex',
                justifyContent: 'space-between',
                alignItems: 'center', // Выравнивание названия и кнопок по центру оси
                gap: 2,
              }}
            >
              {/* Наименование вида метрологического контроля */}
              <Typography
                variant="subtitle2"
                sx={{
                  ...
                  fontWeight: 700,
                  color: 'text.primary',
                  wordBreak: 'break-word', // Защита от длинных слов (Поверка, Калибровка)
                  lineHeight: 1.3,
                }}
              >
                {cleanSpaces(m.name)}
              ...
              {/* Кнопки управления в одну линию, без лишних Divider */}
