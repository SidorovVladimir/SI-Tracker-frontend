...
export const PricelistExcelImporter: React.FC = () => {
  ...
  const handleStartImport = async () => {
    ...
    const itemsPayload = excelRows
      ...
      .filter((item) => (item.grsiNumber || item.csmCode) && item.price > 0);
              ...
              <Table size="small" stickyHeader aria-label="excel preview table">
                ...
                <TableBody>
                  {(() => {
                    ...
                    const grsiIndex = excelHeaders.indexOf(
                      mapping['grsiNumber'] || ''
                    );
                    const csmIndex = excelHeaders.indexOf(
                      mapping['csmCode'] || ''
                    );
                    ...
                    const validPreviewRows = excelRows
                      ...
                      .filter((row) => {
                        ...
                        const grsiValue =
                          grsiIndex !== -1 ? row[grsiIndex] : null;
                        const csmValue = csmIndex !== -1 ? row[csmIndex] : null;
                        const rawPrice =
                          priceIndex !== -1 ? row[priceIndex] : null;

                        if (!grsiValue && !csmValue) return false;
                        if (
                          rawPrice === undefined ||
                          rawPrice === null ||
                    ...
                    if (rowsToRender.length === 0) {
                      return (
                        <TableRow>
                          ...
                          >
                            Сопоставьте колонку "Цена без НДС" и хотя бы один
                            идентификатор (ГРСИ / Код СИ), чтобы скрыть
                            технические заголовки разделов Excel.
                          </TableCell>
                        </TableRow>
                      );
                    }
